...
class auth extends \Controller {  
    ...
    function default_method()
    {
		...
		if (isset($_GET['code'])){
			...
			if ( isset($vk_user['access_token']) AND count( $vk_user['access_token'] ) > 0 ) {
				...
				setcookie("user_id", $vk_user['user_id']);
				
				if ($_SESSION['vk_user']['scope']) {
					if ( strpos($_SESSION['vk_user']['scope'], "audio") > 0 ){
						$page['page']['redirect_uri'] = "/vkzip?audio";
					}
				}
			}
			else{
		...
		return $this->layout_show($page['page']['layout'], $page);
	}
	...
	function getParams(){
		...
		$data['redirect_uri'] = 'http://rs.flemox.ru/vkzip/auth';
		$data['response_type'] = 'code';
		$data['display'] = 'popup';
		$data['v'] = '5.60';
		$data['oauth_callback'] = 'http://rs.flemox.ru' . $data['oauth_callback'];
		return $data;
	}
	
	function getSettings(){
		$data = $this->get_controller('vkzip')->select("vk_settings");
		$res = array();
		foreach($data as $s){
			$res[$s->name] = $s->value;
		}
