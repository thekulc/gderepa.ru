...
class Twig_Extension_Core extends Twig_Extension
{
    ...
    public function getFilters()
    {
        $filters = array(
			...
			'rdate'   => new Twig_Filter_Method($this, 'rdate'),
    ...
    /* Kulc truncate function: Оставляет заданное количество символов строки */
    
    public function truncate($text, $max = 30) {
        $lasSpace = 0;
        if (strlen($text) >= $max){
            $text = substr($text, 0, $max);
            $lasSpace = strrpos($text, ' ');
            $text = substr($text, 0, $lasSpace) . "...";
        }
        return $text;
    }
	
	public function rdate($d, $format = "%B %e, %Y %H:%M")
	{
		...
		$res = str_replace($eng, $rus, $str);
		return $res;
	}
	
	private function getRusMonth($strDate, $month){
		...
		foreach ($MonthNames as $mName){
			if (strpos($strDate, $mName))
				return str_replace();
		}
	}
	
	function rdate1($param, $time=0) {
